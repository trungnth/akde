...
def akde(X, ng=None, grid=None, gam=None):
    ...
    # Expand the range slightly to avoid boundary effects (5%)
    MAX += scaling / 20 
    MIN -= scaling / 20
    scaling = MAX - MIN
    ...
    # Set default grid size based on dimension `d` if not provided
    if ng is None:
        ng = 1024 if d == 1 else 512 if d == 2 else 128 if d == 3 else 64
...
def probfun(x, w, mu, Sig):
    ...
    for k in range(gam):
        if np.any(np.linalg.eigvalsh(Sig[:, :, k]) <= 0):
            Sig[:, :, k] += np.finfo(float).eps * np.eye(d)
        ...
        logpdf = (-0.5 * np.sum((np.dot(x - mu[k, :], np.linalg.inv(L)) ** 2), axis=1) 
                  + np.log(max(w[k], np.finfo(float).eps)) - np.sum(np.log(np.maximum(np.diag(L), np.finfo(float).eps))) 
                  - d * np.log(2 * np.pi) / 2)
...
def regEM(w, mu, Sig, bandwidth, X):
    ...
    for i in range(gam):
        if np.any(np.linalg.eigvalsh(Sig[:, :, i]) <= 0):
            Sig[:, :, i] += np.finfo(float).eps * np.eye(d)
        ...
        log_lh[:, i] = (-0.5 * np.sum(xRinv ** 2, axis=1) - np.sum(np.log(np.maximum(np.diag(L), np.finfo(float).eps)))
                        + np.log(max(w[i], np.finfo(float).eps)) - d * np.log(2 * np.pi) / 2 
    ...
    for i in np.where(w > 0)[0]:
        ...
        Sig[:, :, i] = np.dot(Xcentered.T, Xcentered) / w[i] + (bandwidth ** 2 + np.finfo(float).eps) * np.eye(d)
    # Normalize weights
    w /= np.sum(w)
